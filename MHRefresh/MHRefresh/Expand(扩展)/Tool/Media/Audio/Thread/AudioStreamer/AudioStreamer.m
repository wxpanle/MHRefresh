...
- (BOOL)runLoopShouldExit { //runloop停止条件
	@synchronized(self) {
...
/**
 ...
 @param anErrorCode anErrorCode - the error condition
 */
...
/**
 ...
 @param aStatus aStatus - the error condition
 */
...
/**
 ...
 @return YES if the audio currently playing.
 */
- (BOOL)isPlaying {
...
/**
 ...
 @return YES if the AudioStreamer is waiting for a state transition of some kind.
 */
- (BOOL)isWaiting {
	@synchronized(self) {
		if ([self isFinishing] ||
...
/**
 ...
 @return YES if the AudioStream is in the AS_INITIALIZED state (i.e. isn't doing anything).
 */
- (BOOL)isIdle {
...
/**
 ...
 @return YES if the AudioStream was stopped due to some errror, handled through failWithCodeError.
 */
- (BOOL)isAborted {
    return state == AS_STOPPING && stopReason == AS_STOPPING_ERROR;
}

/**
 Generates a first guess for the file type based on the file's extension

 @param fileExtension fileExtension - the file extension
 @return a file type hint that can be passed to the AudioFileStream
 */
+ (AudioFileTypeID)hintForFileExtension:(NSString *)fileExtension {
    NSString *fileExtensionTemp = [fileExtension lowercaseString];
	AudioFileTypeID fileTypeHint = kAudioFileAAC_ADTSType;
	if ([fileExtensionTemp isEqual:@"mp3"]) {
		fileTypeHint = kAudioFileMP3Type;
	} else if ([fileExtensionTemp isEqual:@"wav"]) {
		fileTypeHint = kAudioFileWAVEType;
	} else if ([fileExtensionTemp isEqual:@"aifc"]) {
		fileTypeHint = kAudioFileAIFCType;
	} else if ([fileExtensionTemp isEqual:@"aiff"]) {
		fileTypeHint = kAudioFileAIFFType;
	} else if ([fileExtensionTemp isEqual:@"m4a"]) {
		fileTypeHint = kAudioFileM4AType;
	} else if ([fileExtensionTemp isEqual:@"mp4"]) {
		fileTypeHint = kAudioFileMPEG4Type;
	} else if ([fileExtensionTemp isEqual:@"caf"]) {
		fileTypeHint = kAudioFileCAFType;
	} else if ([fileExtensionTemp isEqual:@"aac"]) {
...
/**
 Open the audioFileStream to parse data and the fileHandle as the data source.

 @return YES  open success
 */
- (BOOL)openReadStream {
	@synchronized(self) {
...
/**
 This is the start method for the AudioStream thread. This thread is created because it will be blocked when there are no audio buffers idle (and ready  to receive audio data).
  ...
  This method contains bits of the "main" function from Apple's example in AudioFileStreamExample.
 */
- (void)startInternal {
...
cleanup:
	...
	@synchronized(self) {
		...
		if (audioQueue) {
			err = AudioQueueDispose(audioQueue, true); //清除queue
			audioQueue = nil;
			if (err) {
...
- (void)internalSeekToTime:(double)newSeekTime {
    ...
    //字节偏移 = 数据偏移 + （偏移目的时间 / 总时长） * (自己总数 - 音频开始偏移字节数)
	seekByteOffset = dataOffset + (newSeekTime / self.duration) * (fileLength - dataOffset);
		
	// Attempt to leave 1 useful packet at the end of the file (although in reality, this may still seek too far if the file has a long trailer). 尝试调整到一个有用的数据包
	if (seekByteOffset > fileLength - 2 * packetBufferSize) {
		seekByteOffset = fileLength - 2 * packetBufferSize;
	}
	
	// Store the old time from the audio queue and the time that we're seeking to so that we'll know the correct time progress after seeking. 保存seektime 用来计算正确的进程
	...
	// Re-open the file stream. It will request a byte-range starting at seekByteOffset.
	[self openReadStream];
}

/**
 Attempts to seek to the new time. Will be ignored if the bitrate or fileLength are unknown.

 @param newSeekTime newSeekTime - the time to seek to
 */
...
/**
 ...
 @return Will return zero if sampleRate has not yet been detected(检测).
 */
- (double)progress {
	@synchronized(self)	{
...
/**
 the bit rate, if known. Uses packet duration times running bits per packet if available, otherwise it returns the nominal bitrate.

 @return return zero if no useful option available.
 */
...
/**
 ...
 If playback has not started before this method is called, it will toggle the "isPlaying" property so that it is guaranteed to transition to true and back to false
 */
- (void)stop {
	@synchronized(self) {
		if (audioQueue &&
...
/**
 ...
 @param aStream aStream - the network file stream
 @param eventType eventType - the event which triggered this method
 */
- (void)handleReadFromStream:(CFReadStreamRef)aStream
                   eventType:(CFStreamEventType)eventType {
	...
	if (eventType == kCFStreamEventErrorOccurred) {
		...
		// If there is a partially filled buffer, pass it to the AudioQueue for processing
		if (bytesFilled) {
		...
		@synchronized(self) {
			...
			// We left the synchronized section to enqueue the buffer so we must check that we are !finished again before touching the audioQueue
...
/**
 Called from ASPacketsProc and connectionDidFinishLoading to pass filled audio bufffers (filled by ASPacketsProc) to the AudioQueue for playback. This function does not return until a buffer is idle for further filling or the AudioQueue is stopped.
 
 This function is adapted from Apple's example in AudioFileStreamExample with CBR functionality added.
 */
- (void)enqueueBuffer {
	@synchronized(self) {
		...
		if (state == AS_BUFFERING ||
			...
			(state == AS_STOPPED && stopReason == AS_STOPPING_TEMPORARILY)) {
			// Fill all the buffers before starting. This ensures that the AudioFileStream stays a small amount ahead of the AudioQueue to avoid an audio glitch playing streaming files on iPhone SDKs < 3.0
    ...
    //pthread_cond_wait  是利用线程间全局共享的全局变量进行同步的一种机制，主要包括两个动作：一个线程等待 条件变量的条件成立 而挂起，另一个线程使条件成立（给出条件成立信号）。  为了防止竞争，条件变量的使用总是和一个互斥锁结合在一起
}

/**
 Method to create the AudioQueue from the parameters gathered by the AudioFileStream.
 
 Creation is deferred to the handling of the first audio packet (although it could be handled any time after kAudioFileStreamProperty_ReadyToProducePackets is true).
 */
- (void)createQueue {
    //音频采样率
	...
	// start the queue if it has not been started already listen to the "isRunning" property
    //添加属性监听
	...
	UInt32 sizeOfUInt32 = sizeof(UInt32); //获取包上界大小
	...
	// allocate audio queue buffers //开辟缓存空间
...
/**
 ...
 @param inAudioFileStream inAudioFileStream - should be the same as self->audioFileStream
 @param inPropertyID inPropertyID - the property that changed
 @param ioFlags ioFlags - the ioFlags passed in
 */
- (void)handlePropertyChangeForFileStream:(AudioFileStreamID)inAudioFileStream
                     fileStreamPropertyID:(AudioFileStreamPropertyID)inPropertyID
                                  ioFlags:(UInt32 *)ioFlags {
	@synchronized(self) {
		...
		if (inPropertyID == kAudioFileStreamProperty_ReadyToProducePackets) {
			...
			if (audioDataByteCount) { //文件长度由此出确定 正确吗
...
/**
 ...
 @param inInputData inInputData - the packet data
 @param inNumberBytes inNumberBytes - byte size of the data
 @param inNumberPackets inNumberPackets - number of packets in the data
 @param inPacketDescriptions inPacketDescriptions - packet descriptions
 */
- (void)handleAudioPackets:(const void *)inInputData
               numberBytes:(UInt32)inNumberBytes
             numberPackets:(UInt32)inNumberPackets
	...
	@synchronized(self) {
		...
		if (bitRate == 0) {
			// m4a and a few other formats refuse to parse the bitrate so we need to set an "unparseable" condition here. If you know the bitrate (parsed it another way) you can set it on the class if needed.
			bitRate = ~0;
		}
	...
	if (inPacketDescriptions) { //卡包描述符
		...
		for (int i = 0; i < inNumberPackets; ++i) {
			...
			if (processedPacketsCount < BitRateEstimationMaxPackets) {
				processedPacketsSizeTotal += packetSize; //获取总包大小
				processedPacketsCount += 1; //获取总包
			}
			
			@synchronized(self) {
				// If the audio was terminated(终止) before this point, then exit.
				if ([self isFinishing]) {
					return;
				}
				
				if (packetSize > packetBufferSize) {//如果包过大  缓冲区太小  无法放置
			...
			@synchronized(self) {
				...
				// If there was some kind of issue with enqueueBuffer and we didn'tmake space for the new audio data then back out
		...
		while (inNumberBytes) {
			...
			@synchronized(self) {
				...
				// If there was some kind of issue with enqueueBuffer and we didn't make space for the new audio data then back out
...
/**
 ...
 @param inAQ inAQ - the queue
 @param inBuffer inBuffer - the buffer
 */
- (void)handleBufferCompleteForQueue:(AudioQueueRef)inAQ
                              buffer:(AudioQueueBufferRef)inBuffer {
...
/**
 ...
 @param inAQ inAQ - the audio queue
 @param inID inID - the property ID
 */
- (void)handlePropertyChangeForQueue:(AudioQueueRef)inAQ
                          propertyID:(AudioQueuePropertyID)inID {
	...
	@synchronized(self) {
		if (inID == kAudioQueueProperty_IsRunning) {
			if (state == AS_STOPPING) {
				// Should check value of isRunning to ensure this kAudioQueueProperty_IsRunning isn't the *start* of a very short stream
				UInt32 isRunning = 0;
...
/**
 ...
 @param notification notification inAQ - the audio queue inID - the property ID
 */
- (void)handleInterruptionChangeToState:(NSNotification *)notification {
