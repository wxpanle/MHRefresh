...
static void MCSAudioFileStreamPropertyListener(void *inClientData,
                                               ...
                                               UInt32 *ioFlags)
{
    QYAudioFileStream *audioFileStream = (__bridge QYAudioFileStream *)inClientData;
...
@implementation QYAudioFileStream
...
- (BOOL)_openAudioFileStreamWithFileTypeHint:(AudioFileTypeID)fileTypeHint error:(NSError *__autoreleasing *)error
{
    ...
    Boolean isBitRate = 0;
    status = AudioFileStreamGetPropertyInfo(_audioFileStreamID, kAudioFileStreamProperty_BitRate, &size, &isBitRate);
    
    if (isBitRate) {
        DLog(@"允许写");
        UInt32 bitRate = _bitRate * 2;
        NSLog(@"%ud", bitRate);
        status = AudioFileStreamSetProperty(_audioFileStreamID, kAudioFileStreamProperty_BitRate, bitRate, &bitRate);
        if (status == noErr) {
            NSLog(@"设置完成");
        }
    } else {
                  ...
                  packetDescriptions:(AudioStreamPacketDescription *)packetDescriptioins
{
    ...
    for (int i = 0; i < numberOfPackets; ++i)
    {
        ...
        QYParsedAudioData *parsedData = [QYParsedAudioData parsedAudioDataWithBytes:packets + packetOffset
