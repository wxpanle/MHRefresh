...
typedef NS_ENUM(NSInteger, UPGestureRespondType) {
    UPGestureRespondTypeNoRespond,
    UPGestureRespondTypeTap,
    UPGestureRespondTypeLongPressLink,
    UPGestureRespondTypeLongPressUnLink
};

typedef NS_ENUM(NSInteger, UPCalculationGestureWay) {
    UPCalculationGestureWayWayTap,
    UPCalculationGestureWayLongPress
};

#define kLineBreakWordWrapTextWidthScalingFactor (M_PI / M_E)

static CGFloat const kFLOAT_MAX = 100000;

static inline CGFloat FlushFactorWithTextAlignment(NSTextAlignment textAlignment) {
...
static inline NSDictionary * NSAttributedStringAttributesFromLabel(UPEventLabel *label) {
...
static inline NSAttributedString * NSAttributedStringByScalingFontSize(NSAttributedString *attributedString, CGFloat scale) {
    ...
    [mutableAttributedString enumerateAttribute:NSFontAttributeName inRange:NSMakeRange(0, [mutableAttributedString length]) options:0 usingBlock:^(id value, NSRange range, BOOL * __unused stop) {
        ...
        if (font) {
            UIFont *resultFont = [UIFont fontWithDescriptor:font.fontDescriptor size:CGFloat_floor(font.pointSize * scale)];
            [mutableAttributedString removeAttribute:NSFontAttributeName range:range];
            [mutableAttributedString addAttribute:NSFontAttributeName value:resultFont range:range];
        }
    }];
...
static inline CGSize CTFramesetterSuggestFrameSizeForAttributedStringWithConstraints(CTFramesetterRef framesetter, NSAttributedString *attributedString, CGSize size, NSUInteger numberOfLines) {
    ...
    CGSize constraints = CGSizeMake(size.width, kFLOAT_MAX);

    if (numberOfLines == 1) {
        // If there is one line, the size that fits is the full width of the line
        constraints = CGSizeMake(kFLOAT_MAX, kFLOAT_MAX);
    } else if (numberOfLines > 0) {
        // If the line count of the label more than 1, limit the range to size to the number of lines that have been set
        CGMutablePathRef path = CGPathCreateMutable();
        CGPathAddRect(path, NULL, CGRectMake(0.0f, 0.0f, constraints.width, kFLOAT_MAX));
...
@property (readwrite, nonatomic, strong) UPEventLabelLink *activeLink;

- (void) longPressGestureDidFire:(UILongPressGestureRecognizer *)sender;

@end

@implementation UPEventLabel {
...
- (void)updateText:(NSString *)text andHighLightText:(NSArray <NSString *>*)highLightTextArray {
    NSString *urlString = [text copy];
    
    NSMutableArray *checkingResultArray = [[NSMutableArray alloc] initWithCapacity:0];
    
    for (NSString *highLightText in highLightTextArray) {
        NSRange range = [text rangeOfString:highLightText];
        if (range.location != NSNotFound) {
            NSTextCheckingResult *result = [NSTextCheckingResult linkCheckingResultWithRange:range URL:[NSURL URLWithString:highLightText]];
    ...
    for (NSTextCheckingResult *result in checkingResultArray) {
        [self addLinkToURL:result.URL withRange:result.range];
    }
}
...
- (void)layoutUI {
    ...
    [mutableActiveLinkAttributes setObject:[NSNumber numberWithBool:NO] forKey:NSUnderlineStyleAttributeName];
    ...
    _up_contentMode = UPEventLabelContentModeCenter;
}
...
- (void)setActiveLink:(UPEventLabelLink *)activeLink {
...
- (void)addLinks:(NSArray *)links {
    ...
    for (UPEventLabelLink *link in links) {
        if (link.attributes) {
...
- (NSArray *)addLinksWithTextCheckingResults:(NSArray *)results
    ...
    for (NSTextCheckingResult *result in results) {
        ...
        UPEventLabelLink *link = [[UPEventLabelLink alloc] initWithAttributes:attributes
...
- (UPGestureRespondType)containslinkAtPoint:(CGPoint)point {
    return [self linkAtPoint:point withCalculationGestureWay:UPCalculationGestureWayLongPress];
}

- (UPGestureRespondType)linkAtPoint:(CGPoint)point withCalculationGestureWay:(UPCalculationGestureWay)way {
    ...
    if (!CGRectContainsPoint(CGRectInset(self.bounds, -15.f, -15.f), point) || self.links.count == 0) {
        self.activeLink = nil;
        return UPGestureRespondTypeNoRespond;
    }

    UPEventLabelLink *result = [self linkAtCharacterIndex:[self characterIndexAtPoint:point]];
    ...
    self.activeLink = result;
    
    switch (way) {
        case UPCalculationGestureWayWayTap:
            return result == nil ? UPGestureRespondTypeNoRespond : UPGestureRespondTypeTap;
            break;
            
        case UPCalculationGestureWayLongPress:
            return result == nil ? UPGestureRespondTypeLongPressUnLink : UPGestureRespondTypeLongPressLink;
            break;
        default:
            break;
    }
...
- (UPEventLabelLink *)linkAtRadius:(const CGFloat)radius aroundPoint:(CGPoint)point {
...
- (UPEventLabelLink *)linkAtCharacterIndex:(CFIndex)idx {
...
- (CFIndex)characterIndexAtPoint:(CGPoint)p {
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        CGFloat flushFactor = FlushFactorWithTextAlignment(self.textAlignment);
...
- (void)drawTextInRect:(CGRect)rect {
    ...
    if (self.adjustsFontSizeToFitWidth && self.numberOfLines > 0) {
        ...
        CGSize maxSize = (self.numberOfLines > 1) ? CGSizeMake(kFLOAT_MAX, kFLOAT_MAX) : CGSizeZero;
        ...
        if (self.numberOfLines > 1 && self.lineBreakMode == NSLineBreakByWordWrapping) {
            textWidth *= kLineBreakWordWrapTextWidthScalingFactor;
        }
...
- (void)drawBackground:(CTFrameRef)frame
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            ...
            CGColorRef strokeColor = CGColorRefFromColor([attributes objectForKey:NSBackgroundColorAttributeName]);
            CGColorRef fillColor = CGColorRefFromColor([attributes objectForKey:NSBackgroundColorAttributeName]);
            UIEdgeInsets fillPadding = UIEdgeInsetsZero;
            CGFloat cornerRadius = 2.0;
            CGFloat lineWidth = UPDefaultLineNarrow;
...
- (void)drawStrike:(CTFrameRef)frame
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            NSDictionary *attributes = (__bridge NSDictionary *)CTRunGetAttributes((__bridge CTRunRef) glyphRun);
            BOOL strikeOut = [[attributes objectForKey:NSStrikethroughStyleAttributeName] boolValue];
...
- (CGRect)textRectForBounds:(CGRect)bounds
    ...
    if (textSize.height < bounds.size.height) {
        ...
        switch (self.up_contentMode) {
            case UPEventLabelContentModeCenter:
                yOffset = CGFloat_floor((bounds.size.height - textSize.height) / 2.0);
                break;
                
            case UPEventLabelContentModeBottom:
                yOffset = bounds.size.height - textSize.height;
                break;
                
            case UPEventLabelContentModeTop:
            case UPEventLabelContentModeLeft:
            case UPEventLabelContentModeRight:
...
- (void)tintColorDidChange {
    ...
    NSMutableAttributedString *mutableAttributedString = [self.attributedText mutableCopy];
    for (UPEventLabelLink *link in self.linkModels) {
...
- (UIView *)hitTest:(CGPoint)point
    ...
    if ([self linkAtPoint:point withCalculationGestureWay:UPCalculationGestureWayWayTap] == UPGestureRespondTypeNoRespond || self.userInteractionEnabled == NO || self.hidden || self.alpha < 0.01) {
...
- (void)copyLabel:(id)sender {
    ...
    if (_eventDelegate && [_eventDelegate respondsToSelector:@selector(up_eventLabelCopyEvent:)]) {
        [_eventDelegate up_eventLabelCopyEvent:self];
    }
}
...
- (void)touchesBegan:(NSSet *)touches
    ...
    [self linkAtPoint:[touch locationInView:self] withCalculationGestureWay:UPCalculationGestureWayWayTap];
    ...
    if (self.activeLink) {
        ...
        switch (result.resultType) {
            case NSTextCheckingTypeLink:
                ...
                if (_eventDelegate && [_eventDelegate respondsToSelector:@selector(up_eventLabel:selectedLink:)]) {
                    [_eventDelegate up_eventLabel:self selectedLink:result.URL];
                }
...
- (void)longPressGestureDidFire:(UILongPressGestureRecognizer *)sender {
    switch (sender.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            UPGestureRespondType type = [self linkAtPoint:touchPoint withCalculationGestureWay:UPCalculationGestureWayLongPress];
            
            if (self.activeLink) {
                if (self.activeLink.linkLongPressBlock) {
                    self.activeLink.linkLongPressBlock(self, self.activeLink);
                    return;
                }
            }
            
            switch (type) {
                case UPGestureRespondTypeLongPressUnLink: {
                    [self copyLinkText];
                }
                    break;
                    
                case UPGestureRespondTypeLongPressLink: {
                    ...
                    if (self.activeLink) {
                        ...
                        if (result.resultType == NSTextCheckingTypeLink) {
                            ...
                            if (_eventDelegate && [_eventDelegate respondsToSelector:@selector(up_eventLabel:longPressLink:point:)]) {
                                [_eventDelegate up_eventLabel:self longPressLink:result.URL point:touchPoint];
                            }
                        } else {
                            [self copyLinkText];
                        }
                    ...
                    break;
                case UPGestureRespondTypeNoRespond: {
                    
                    break;
                }
                case UPGestureRespondTypeTap: {
...
#pragma mark - ===== UPEventLabelLink =====

@implementation UPEventLabelLink
