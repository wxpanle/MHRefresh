...
#import "UIView+QYVisuals.h"

#define degToRad(x) (M_PI * (x) / 180.0)

@implementation UIView (QYVisuals)

- (void)cornerRadius:(CGFloat)radius strokeWidth:(CGFloat)width color:(UIColor *)color {
    
    self.layer.cornerRadius = radius;
    self.layer.borderColor = nil == color ? [UIColor lightGrayColor].CGColor : color.CGColor;
...
- (void)shadowWithColor:(UIColor *)color offset:(CGSize)offset opacity:(CGFloat)opacity radius:(CGFloat)radius {
    ...
    self.layer.shadowColor = color == nil ? [UIColor lightGrayColor].CGColor : color.CGColor;
