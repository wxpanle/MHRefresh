...
@implementation NSString (QYEmoji)

- (BOOL)isIncludingEmoji {
    return [[self class] isIncludingEmoji:self];
}
    ...
    [originText enumerateSubstringsInRange:NSMakeRange(0, originText.length) options:NSStringEnumerationByComposedCharacterSequences usingBlock:^(NSString * _Nullable substring, NSRange substringRange, NSRange enclosingRange, BOOL * _Nonnull stop) {
        ...
        if (0xd800 <= hs && hs <= 0xdbff) {
            if (substring.length > 1) {
                ...
                const int uc = ((hs - 0xd800) * 0x400) + (ls - 0xdc00) + 0x10000;
                if (0x1d000 <= uc && uc <= 0x1f77f) {
                    [originText replaceCharactersInRange:substringRange withString:replaceText];
                }
            }
        } else if (substring.length > 1) {
            const unichar ls = [substring characterAtIndex:1];
...
+ (NSString *)clearEmojiWithText:(NSString *)text {
    return [self replaceEmojiWithText:text replaceText:@""];
}

- (NSString *)clearEmoji {
    return [[self class] replaceEmojiWithText:self replaceText:@""];
}

- (NSString *)replaceEmojiWithText:(NSString *)text {
    return [[self class] replaceEmojiWithText:self replaceText:text];
}
