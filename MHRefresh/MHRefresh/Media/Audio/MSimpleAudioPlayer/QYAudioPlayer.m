...
#import "QYAudioFileStream.h"
#import "QYAudioOutputQueue.h"
#import "QYAudioBuffer.h"
#import <pthread.h>

@interface QYAudioPlayer () <MCAudioFileStreamDelegate> {
    NSThread *_thread;
    ...
    UInt32 _bufferSize;
    QYAudioBuffer *_buffer;
    
    QYAudioFile *_audioFile;
    QYAudioFileStream *_audioFileStream;
    QYAudioOutputQueue *_audioQueue;
...
- (BOOL)createAudioQueue
{
    ...
    if (_bufferSize > 0)
    {
        ...
        _audioQueue = [[QYAudioOutputQueue alloc] initWithFormat:format bufferSize:_bufferSize macgicCookie:magicCookie];
...
- (void)threadMain
{
    ...
    if ([QYAudioSession setAudioSessionCategoryPlaybackIfNeeded])
    {
        //active audiosession
//        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(interruptHandler:) name:MCAudioSessionInterruptionNotification object:nil];
        if ([QYAudioSession setAudioSessionActive:YES])
        {
            //create audioFileStream
            _audioFileStream = [[QYAudioFileStream alloc] initWithFileType:_fileType fileSize:_fileSize error:&error];
            if (!error)
            {
                _failed = NO;
    ...
    while (self.status != MCSAPStatusStopped && !_failed && _started)
    {
        @autoreleasepool
        {
            ...
            if (_usingAudioFile)
            {
                if (!_audioFile)
                {
                    _audioFile = [[QYAudioFile alloc] initWithFilePath:_filePath fileType:_fileType];
                }
...
- (void)play
{
    ...
    else
    {
        if (_status == MCSAPStatusPaused || _pauseRequired)
        {
            ...
            if ([QYAudioSession setAudioSessionActive:YES])
            {
                [QYAudioSession setAudioSessionCategoryPlaybackIfNeeded];
                [self _resume];
            }
        }
    }
...
- (void)printfDuration {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        DLog(@"%f, %f", strongSelf.duration, strongSelf.progress);
        [self printfDuration];
    });
}
