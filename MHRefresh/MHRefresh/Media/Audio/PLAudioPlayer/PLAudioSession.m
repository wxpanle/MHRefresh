...
@interface PLAudioSession () {
    NSHashTable *_observesTable;
}

@end

@implementation PLAudioSession

#pragma mark - ======== init ========

+ (instancetype)shardeInstance {
    static PLAudioSession *audioSession = nil;
...
- (void)addAudioSessionDelegate:(id <PLAudioSessionDelegate>)delegate {
    
    if ([delegate conformsToProtocol:@protocol(PLAudioSessionDelegate)] &&
        ![_observesTable containsObject:delegate]) {
        [_observesTable addObject:delegate];
    }
}

- (void)removeAudioSessionDelegate:(id <PLAudioSessionDelegate>)delegate {
...
+ (BOOL)setAudioSessionActive:(BOOL)active options:(AVAudioSessionSetActiveOptions)options {
    BOOL success = NO;
    __autoreleasing NSError *error = nil;
    success = [[AVAudioSession sharedInstance] setActive:YES withOptions:options error:&error];
    if (success == NO || error) {
        DLog(@"setActive: failure, error: %@", error);
    }
    return success;
...
- (void)audioSessionDidChangeProperty:(NSNotification*)notification {
    ...
    for (id <PLAudioSessionDelegate> observer in _observesTable) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)audioSessionInterrupt:(NSNotification*)notification {
    ...
    for (id <PLAudioSessionDelegate> observer in _observesTable) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([observer respondsToSelector:@selector(audioInterruptNotification:)]) {
                [observer audioInterruptNotification:notification];
            }
        });
    }
